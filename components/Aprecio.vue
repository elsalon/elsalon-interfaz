<template>
    <div class="relative group/aprecio w-max flex">
        <!-- Btn Aprecio -->
         <div class="relative">
             <Button link class="my-2 mr-1 text-xs text-zinc-600 dark:text-zinc-400 leading-normal" :class="{ 'font-bold text-zinc-800': haApreciadoShowState }" style="padding: 0"
        ...
        <!-- Btn Cantidad -->
        <div v-show="totalDocs == 0" class="inline-block my-2 font-mono text-xs  text-zinc-600 dark:text-zinc-400 leading-normal"
            :class="{ 'opacity-30': fetching }">(0)</div>

        <Button v-show="totalDocs > 0" v-tooltip.top="userNamesTooltip"  link class="my-2 text-xs  text-zinc-600 dark:text-zinc-400 leading-normal"
