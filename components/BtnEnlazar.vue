<template>
    <div class="text-center" v-if="estadoMostrarBtnEnlace > 0">
        <Button v-if="estadoMostrarBtnEnlace==1" @click="IniciarColaboracion" :loading="loading" :label="loading? 'Enlazando' : 'Enlazar'" :title="title" size="small"/>
        <Button v-if="estadoMostrarBtnEnlace==2" @click="DetenerColaboracion" :loading="loading" severity="secondary" :label="loading? 'Desenlazando' : 'Desenlazar'" size="small"></Button>
    </div>
</template>
...
<script setup>
    ...
    const estadoMostrarBtnEnlace = ref(0) // 0 no se muestra nada, 1 se muestra colaborar, 2 se muestra dejar de colaborar
    ...
    const resColaboracion = await useAPI(`/api/enlaces?where[autor][equals]=${auth.data.value.user.id}&where[idEnlazado][equals]=${SalonStore.contextoId}`)
    if(resColaboracion.docs.length > 0){
        idEnlace.value = resColaboracion.docs[0].id
        estadoMostrarBtnEnlace.value = 2
    }else{
        estadoMostrarBtnEnlace.value = 1
    }
