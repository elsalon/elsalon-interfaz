    ...
    <article>
            ...
            <!-- <EntradaDefault /> -->
                ...
                <div class="ml-4">
                    <NuxtLink :to="identidadUrl" class="hover:underline">
                        <h2 class="font-bold text-zinc-900 dark:text-zinc-100" v-tooltip.top="tooltipIdentidad">{{ identidad.nombre }}</h2>
                    </NuxtLink>
                    <div class="flex items-center">
                        <NuxtLink v-if="entrada.sala" class="text-sm mr-2 hover:underline text-zinc-600 dark:text-zinc-400" 
                            :to="`/salones/${entrada.sala.slug}`">{{ entrada.sala.nombre }}</NuxtLink>
                        <NuxtLink v-else="identidadUrl" class="text-sm mr-2 hover:underline text-zinc-600 dark:text-zinc-400" :to="identidadUrl">Bitácora</NuxtLink>
                        <NuxtLink class="text-zinc-600 dark:text-zinc-400 text-sm hover:underline" :to="`/entradas/${entrada.id}`">
                            <time :datetime="entrada.createdAt" class="text-zinc-600 dark:text-zinc-400 text-xs">{{ $formatDate(entrada.createdAt) }}</time>
                        </NuxtLink>
                        <!-- Entrada Fijada -->
        ...
        <div class="despues-entrada ml-0">
            ...
            <div class="actions">
                ...
                <Button v-show="!listaComentarios?.comentarios?.length > 0" link class="my-2 mr-2 text-xs text-zinc-600 dark:text-zinc-400 leading-normal" style="padding: 0;"
